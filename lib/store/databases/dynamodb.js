...
_.extend(DynamoDB.prototype, {
  ...
  save: function (saga, cmds, callback) {
    ...
    console.log("#################### JINGJIBO 02 ############## ", cmds)
    if (cmds) {
      console.log("#################### JINGJIBO 03 ############## ")
      saga._commands = cmds.reduce((result, cmd) => {
        result[cmd.id] = cmd;
        return result;
      } , {});
      console.log("#################### JINGJIBO 04 ############## ", saga._commands)
    }
    
    if (saga._timeoutAt) {
      saga._timeoutAt = saga._timeoutAt.getTime();
      saga._timeoutAtYearMonth = new Date().toISOString().slice(0, 7).replace("-", "");
    } 

    if (saga._timeoutCommands) {
      saga._timeoutCommands = saga._timeoutCommands.reduce((result, cmd) => {
        result[cmd.id] = cmd;
        return result;
      } , {});
      console.log("#################### JINGJIBO 04 ss ############## ", saga._timeoutCommands)
    }

    if (saga._commitStamp) {
      saga._commitStamp = saga._commitStamp.getTime();
    ...
    if (!saga._hash) {
      ...
      console.log("#################### JINGJIBO 05 ############## ", params)
  ...
  get: function (id, callback) {
    ...
    self.documentClient.get(params, function(err, data) {
      ...
      if (saga._timeoutCommands) {
        saga._timeoutCommands = Object.values(saga._timeoutCommands);
      }
  ...
  getTimeoutedSagas: function (options, callback) {
    ...
    self.documentClient.scan(params, function(err, data) {
      ...
      sagas.forEach(function (s) {
        ...
        if (s._timeoutCommands) {
          s._timeoutCommands = Object.values(s._timeoutCommands);
        }
  ...
  setCommandToDispatched: function (cmdId, sagaId, callback) {
    console.log("///////////// CINGCINGPOK 01 ", cmdId, sagaId)
    ...
    console.log("///////////// CINGCINGPOK 02 ", params)

    self.documentClient.update(params, function(err, data) {
      console.log("///////////// CINGCINGPOK 03 ", err, data)
      if (callback) {
        callback(err);
      }
    });
